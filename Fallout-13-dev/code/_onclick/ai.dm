...
/mob/living/silicon/ai/ClickOn(var/atom/A, params)
	...
	if(pixel_turf)
		...
		if(!turf_visible)
			log_admin("[key_name_admin(src)] might be running a modified client! (failed checkTurfVis on AI click of [A]([ADMIN_COORD(pixel_turf)])")
			...
			send2irc_adminless_only("NOCHEAT", "[key_name(src)] might be running a modified client! (failed checkTurfVis on AI click of [A]([ADMIN_COORD(pixel_turf)]))")
			return
